...
** the org notes are now these
   The top-level README needs to be amended to point to them.
...
** rename shell.json to config.json
   and repl.json to config.repl.json.
...
** TODO is alimony a cost to someone?
   It should be.
** TODO understand this absurdly strange bug
