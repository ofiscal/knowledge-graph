...
** tax.co.web, some websites it hosts
*** manual_ingest
...
*** write_time (for testing)
    ...
    http://ofiscal-puj.org/myapp/run_make/write_time
* PITFALLs
...
* TODO [[id:7f3be852-18ca-4381-8462-c6425cfa37ca][Mailgun & tax.co.web]]
* TODO Are there still purchases with vat paid > 0 and "value, purchase" == 0?
** There were before the bugfix.
   They are all COICOP-encoded;
     the capitulo C stuff shows no such problems.
   They all use COICOP code 12700601, which is the predial:
     "Impuesto predial y de valorizaci�n de la(s) vivienda(s) ocupada(s) por el hogar,12700601,0.0,0.0,0.0"
** definitions
*** purchases["value"] =	      (in purchases_2_vat.py)
    purchases["per month"] * purchases["value"]
*** purchases["vat paid"] =	      (in purchases_2_vat.py)
    purchases["value"] * purchases["vat frac"]
*** purchases["value, purchase"] =    (in purchase_sums.py)
    (purchases[ "is-purchase" ] > 0) * purchases["value"]
*** merge["vat / purchase value" ] =  (in households_2_purchases.py)
    merge["vat paid"] / merge["value, purchase" ]
*** purchases["is-purchase"] is (renamed but) defined very early
** More notes from during Thursday's bugfix.
   "vat / purchase value" is too big sometimes
**** ??? In purchase_sums.py, vat paid is zeroed for non-purchases!
     for col in vat_columns:
       purchases[col] = (
	 (purchases[ "is-purchase" ] > 0) *
	 purchases[col] )
**** The bug shows up in households_2_purchases_test.py
     around this line:
       "vat / purchase value"      : cl.InRange( 0, 0.3 ),
**** the weird households
  hs = ( merge
	 [ merge["vat / purchase value"] > 0.3 ]
	 [ "household" ] )
       == [ 215095
          , 524310
	  , 598117
	  , 610546 ]
* TODO Why is Gmail blocking me again?
* TODO things we define and don't use
  We don't use the last two of these.
  (They're all defined in the same place, purchase_sums.py.)

  "value, tax, predial"
  "value, purchase"
  "value, tax, purchaselike non-predial non-VAT"
  "value, non-purchase"
* TODO [[id:2d8ca73e-da41-4876-9a8e-982164228839][tax.co person-level analysis]]
