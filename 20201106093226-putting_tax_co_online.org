...
***** Configure Security Group
      ...
      HTTPS (added)
*** DONE login
    ssh -i ~/code/aws/secret/2021-04-14.1.pem ec2-user@18.228.171.239
...
*** DONE install docker, emacs
    sudo yum install -y docker emacs
**** first time I tried, all docker dependencies were already installed
    sudo yum install -y yum-utils device-mapper-persistent-data lvm2
*** transfer from home machine
    scp takes a long time for big files.
**** ~/tax_co.docker.tar
     scp -i ~/code/aws/secret/2021-04-14.1.pem ~/tax_co.docker.tar ec2-user@18.228.171.239:~
     docker import tax_co.docker.tar
**** DONE ~/of/webapp.zip
     scp -i ~/code/aws/secret/2021-04-14.1.pem ~/of/webapp.zip ec2-user@18.228.171.239:~
     unzip webapp.zip
**** ~/of/tax.co/web.zip
*** DONE configure Docker not to need sudo
    sudo usermod -aG docker ec2-user
    then close ssh and ssh back in
*** screw with filepaths
**** Take notes here of what needs changing.
     webapp/commands.sh
       change jeff -> ec2-user
**** Eventually make those changes programmatic.
*** visit something like this URL
    sim.jefbrown.net/myapp/run_make/write_time
