...
** TODO serial-ish
*** TODO pass a list from Python to Javascript HTML
    Will need this to create the marginal rate tables from the .csv files.
*** TODO show the default marginal tax rates in the UI
    alternatives
**** use the `csv` package rather than pandas from Django
...
*** TODO create, store user .csv files from marginal rate tables
** email + subsample + radio table + dynamic marginnal tables
*** Comment out the other configuration stuff.
