...
*** TODO BLOCKED add Haskell files
...
**** TODO BLOCKED: if db already has request from user
     blocked on Django
***** TODO initial hack
      Just assume they want the earlier result deleted.
***** what to do
     Ask if they want to delete the old one and replace it with the new one.
     Explain they can only have one in the db at a time.
**** create folder, move db and spec to it
**** modify db of requests
...
***** Run if both
      (1) either (a) a result is more than an hour old, or
                 (b) there's room for another run
      (2) the db contains unexecuted requests
...
***** all output should go to a specific user's folder
****** TODO unduplicate the subsamples across users
****** TODO ? unduplicate the other non-user-specific files
