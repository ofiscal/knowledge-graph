...
* TODO blocking
** TODO BUG: circular import with pandas
   Rebuilding the Docker container may have solved it.

   Somoene online suggests that pandas might be corrupted, try reinstalling. I'm running `conda update -y pandas` as root (otherwise it fails).
   If it works, then I should be able to restore Makefile.tests to what it was.
